...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadSessions();  // Load past sessions
...
function initializeEventListeners() {
    ...
    // Sessions Toggle
    document.getElementById('enable-sessions').addEventListener('change', (e) => {
        const sessionsList = document.getElementById('sessions-list');
        sessionsList.classList.toggle('active', e.target.checked);
        if (e.target.checked && sessionsList.children.length === 0) {
            loadSessions();  // Load sessions when first enabled
        }
    });
...
async function handleFileUpload(event) {
    ...
    statusDiv.innerHTML = `<span>⏳ Uploading ${file.name}...</span>`;
    ...
    try {
        ...
        statusDiv.innerHTML = `<span>✅ ${data.filename}</span>`;
        ...
        statusDiv.innerHTML = `<span>❌ ${error.message}</span>`;
...
function addUploadedDocToChat(uploadData) {
    ...
    exchangeDiv.innerHTML = `
        <div class="exchange-header">
            <span class="exchange-summary">📄 ${uploadData.filename}</span>
...
async function sendMessage(queryText = null) {
    ...
    try {
        ...
        // Use streaming endpoint (same as v2)
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (data.type === 'session_id') {
                        ...
                        tokenCount++;
                        appendToStreamingMessage(streamingId, data.content);                    } else if (data.type === 'distortion') {
                        // Single distortion output - replace main response
                        console.log('Distortion received, replacing content');
                        replaceStreamingMessageContent(streamingId, data.content);
                    } else if (data.type === 'ensemble') {
                        // Ensemble distortion outputs received
                        console.log('Ensemble outputs received:', data.outputs?.length);
                        displayEnsembleOutputs(data.outputs);                    } else if (data.type === 'distorted_query') {
...
function startNewExchange() {
    ...
    exchangeDiv.innerHTML = `
        <div class="exchange-header">
            <span class="exchange-summary">💬 New Query</span>
            <span class="exchange-toggle">▼</span>
        </div>
        <div class="exchange-content">
        </div>
...
// Display ensemble distortion outputs
function displayEnsembleOutputs(outputs) {
    if (!outputs || outputs.length === 0) return;
    
    console.log('Displaying ensemble outputs:', outputs);
    
    const messagesContainer = document.getElementById('chat-messages');
    const targetContainer = currentExchange ? currentExchange.querySelector('.exchange-content') : messagesContainer;
    
    outputs.forEach((output, index) => {
        console.log(`Ensemble ${index}:`, output.mode, 'content length:', output.content?.length);
        
        const messageDiv = document.createElement('div');
        messageDiv.className = 'message assistant ensemble-output';
        ...
        header.textContent = `${output.mode} (${output.tone}, gain ${output.gain})`;
        
        contentDiv.appendChild(header);
        
        // Parse markdown for the content - use output.response
        const textContent = output.response || output.content || output.distorted_text || '';
        console.log(`Setting content for ${output.mode}, length: ${textContent.length}`);
        if (textContent && typeof marked !== 'undefined') {
            ...
            const textDiv = document.createElement('div');
            textDiv.textContent = textContent || '(No content)';
            contentDiv.appendChild(textDiv);
        }
...
function finalizeStreamingMessage(messageId, contextData = null) {
    ...
    if (!text || text.length === 0) {
        contentDiv.textContent = '⚠️ No response received from LLM.';
        return;
    }
...
async function loadNewsArticle(filename) {
    try {
        ...
        statusDiv.innerHTML = `<span>✅ Loaded: ${filename}</span>`;
...
async function loadSessions() {
    try {
        ...
        console.log('Loaded', sessions.length, 'past sessions');
    } catch (error) {
        ...
        for (let i = 0; i < sessionData.messages.length; i++) {
            ...
            if (msg.role === 'user') {
                ...
                if (currentExchange && i < sessionData.messages.length - 2) {
                    const content = currentExchange.querySelector('.exchange-content');
                    const toggle = currentExchange.querySelector('.exchange-toggle');
                    content.classList.add('collapsed');
                    if (toggle) toggle.textContent = '▶';
                }
            }
